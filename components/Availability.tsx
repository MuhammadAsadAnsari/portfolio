...
function Availability() {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const EMAILJS_SERVICE_ID = 'service_lzrtaz6';
      const EMAILJS_TEMPLATE_ID = 'template_k36jket';
      const EMAILJS_PUBLIC_KEY = 's0Cg6EcZLlNBilPEB';
  ...
  return (
    <section id="contact" className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-12 sm:py-14 md:py-16 lg:py-20 scroll-mt-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-7 md:gap-8 mt-6 sm:mt-8">
        ...
        <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-slate-800 rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-7 backdrop-blur-sm border border-indigo-500/30 shadow-indigo-500/20">
          <div className="space-y-5 sm:space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 disabled:from-gray-500 disabled:to-gray-600 disabled:cursor-not-allowed text-white font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-md sm:rounded-lg text-sm sm:text-base transition-all duration-300 transform hover:scale-105 disabled:hover:scale-100"
              >
              ...
              {submitStatus === 'error' && (
                <div className="text-center text-red-400 text-xs sm:text-sm bg-red-400/10 rounded-md sm:rounded-lg p-2.5 sm:p-3 border border-red-400/20">
                  Failed to send message. Please check your email format and try again.
                </div>
              )}
            </form>
          </div>
