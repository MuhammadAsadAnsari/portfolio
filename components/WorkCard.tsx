...
function WorkCard({ title, company, subtitle, period, children, image, iconSrc, customIcon }:
  { title: string; subtitle: string; company: string; period: string; children?: ReactNode; image?: string; iconSrc?: string | null; customIcon?: ReactNode; }) {
  return (
        ...
        <div className="h-16 w-16 sm:h-20 sm:w-20 md:h-24 md:w-24 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#2A1748] to-[#0F0A1A] ring-1 ring-white/10 grid place-items-center overflow-hidden shadow-[0_12px_34px_rgba(0,0,0,0.45)]">
          {customIcon ? (
            ...
            <Image 
              ...
              width={88} 
              height={88} 
              className="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 object-cover rounded-xl"
              loading="lazy"
              quality={85}
            />
          ) : (
