...
const Skills = () => {
  ...
  return (
    <section id="skills" className="relative mx-auto max-w-7xl px-4 py-20 scroll-mt-20">
      {/* Subtle Background */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-8">
              {skillCategories.slice(0, 2).map((category, categoryIdx) => (
                ...
                >
                  {/* Category Box */}
                  <div className="bg-slate-800/30 backdrop-blur-sm rounded-xl border border-slate-600/20 p-6 hover:bg-slate-800/40 transition-all duration-300">
                    {/* Category Header */}
                    <div className="mb-4">
            ...
            >
                          {/* Technology Card */}
                          <div 
                            className="relative bg-slate-700/50 backdrop-blur-sm rounded-lg border border-slate-500/30 px-3 py-2 text-center transform group-hover/skill:-translate-y-1 group-hover/skill:scale-105 transition-all duration-300 hover:shadow-lg"
                style={{
                              boxShadow: hoveredSkill === `${category.name}-${skill.name}` ? `0 4px 20px ${skill.color}20` : `0 2px 10px rgba(0,0,0,0.3)`,
                            }}
                          >
                            {/* Technology Name */}
                            <h3 className="text-sm font-bold text-white group-hover/skill:text-white transition-colors">
                              {skill.name}
                            </h3>

                            {/* Tooltip with Description */}
                            <div 
                              className="absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-slate-900/90 backdrop-blur-sm rounded-lg text-xs text-white/90 opacity-0 group-hover/skill:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-10 border border-slate-600/30"
                            >
                              {skill.description}
